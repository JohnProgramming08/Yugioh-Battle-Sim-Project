...
const backCardImage =
  "https://th.bing.com/th/id/OIP.0x4CeXmIq4eMaU3Xqgo2lAAAAA?w=203&h=295&c=7&r=0&o=5&dpr=1.5&pid=1.7";
...
class MonsterSelect {
  constructor(cards, title, nextPage, drawDiv, downCard) {
    ...
    this.defeatedMonsters = JSON.parse(
      localStorage.getItem("defeatedMonsters")
    );
    this.cards = cards;
    this.title = title;
  ...
  cardEventListeners() {
    ...
    for (const card of shownMonsterCards) {
      card.addEventListener("click", (e) => {
        ...
        let targetEl =
          e.target.parentElement.id !== "m-card"
        ...
        for (const key in this.chosenCards) {
          ...
          if (image === cardImage.src) {
            ...
            localStorage.setItem(
              "selectedMonster",
              JSON.stringify(this.selectedMonster)
            );
          }
        }
        this.flipCard();
    ...
    try {
      ...
      if (enemyMonster.health === 0) {
        ...
        this.title.innerHTML =
          "Choose another monster to finish off your opponent!";
      }
    } catch (e) {
      const enemyMonster = this.randomCard();
      localStorage.setItem("enemyMonster", JSON.stringify(enemyMonster));
...
if (battleStarted) {
  ...
  cardsDiv.innerHTML = "";
  const monsterSelect = new MonsterSelect(
    cardsDiv,
    title,
    nextPageBtn,
    drawCard,
